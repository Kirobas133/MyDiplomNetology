	...
	bool skobki(const string& str) {// Проверка секции
		for (const auto& x : str) {
			if (x == '[' || x == ']') {
				...
				else throw "Cинтаксическая ошибка в строке \"" + str + "\" отсутсвует скобка ";
			}
		}
		return false;
	}
	void ravno(const string& str) {// Проверка равно
		int chetchik = 0;
		for (const auto& s : str)
			if (s == '=')
				++chetchik;
		if (chetchik == 0)
			throw "Cинтаксическая ошибка в строке \"" + str + "\" отсутсвует знак '='";			
	}
	void  sekciya_perem_znach(const string& str, string& perem, string& znach,char simvol) {
		...
		for (const auto& s : str) {
			if (s != simvol && flag)
				...
				if (flag) {//если встретили првое равно пропускаем, все следующие это один из символов строки
					flag = false;
					continue;
				}
				znach += s;
